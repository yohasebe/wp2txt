...
Wikipediaダンプからプレーンテキストを抽出するツールはいくつかあります。[WikiExtractor](https://github.com/attardi/wikiextractor)は高速で有名なPythonベースのツールです。しかし、**wp2txtはWikiExtractorにはない独自の機能を提供**します：

| 機能 | wp2txt | WikiExtractor |
|------|--------|---------------|
| プレーンテキスト抽出 | ✅ | ✅ |
| **カテゴリメタデータ抽出** | ✅ | ❌ |
| **カテゴリベースの記事抽出** | ✅ | ❌ |
| カテゴリのみ出力 (`-g`) | ✅ | ❌ |
| **タイトル指定による記事抽出** | ✅ | ❌ |
| セクション見出し | `==タイトル==` (カスタマイズ可能) | `Title.` (固定形式) |
| 多言語カテゴリ対応 | ✅ (350+言語) | — |
| 処理速度 | やや遅い | 約10倍高速 |

### wp2txtを使うべき場合

- 分類やナレッジグラフ構築に**記事のカテゴリ情報**が必要
- **特定カテゴリの記事を抽出**したい（例：「日本の都市」カテゴリの全記事）
- フルダンプをダウンロードせずに**特定の記事をタイトルで抽出**したい
- セクション見出しの形式を維持またはカスタマイズしたい
- カテゴリをラベルとしてトピック分類器を構築している
- 処理時間が最優先事項ではない

### WikiExtractorを使うべき場合

- プレーンテキストコンテンツのみが必要（メタデータ不要）
- 処理速度が重要
- フルWikipediaダンプ（20GB以上）を扱う

## 責任あるデータアクセス

**wp2txtは公式のWikipediaダンプファイルを使用**します。これはWikimedia Foundationが[推奨するバルクデータアクセス方法](https://meta.wikimedia.org/wiki/Data_dumps)です。

### なぜAPIスクレイピングではなくダンプファイル？

Wikimedia Foundationは、特にAI/ML目的での大規模APIスクレイピングについて懸念を表明しています：

- **サーバー負荷**: 大量のAPIリクエストはWikipediaのインフラに負担をかける
- **公式推奨**: ダンプファイルはバルクデータアクセス用に提供されている
- **利用規約**: 過剰なボットリクエストはブロックされる可能性がある

wp2txtの設計はこれらのガイドラインに準拠しています：

| アプローチ | サーバー影響 | Wikimediaの見解 |
|-----------|-------------|-----------------|
| APIスクレイピング（大量） | 高 | ⚠️ 非推奨 |
| **ダンプファイル（wp2txt）** | なし | ✅ 推奨 |

ダンプファイルを使用することで、wp2txtはWikimediaのインフラとポリシーを尊重しながら大規模なWikipediaデータ抽出を可能にします。

## 概要

WP2TXTはWikipediaダンプファイル（XMLエンコード/Bzip2圧縮）からテキストとカテゴリデータを抽出し、MediaWikiマークアップやその他のメタデータを除去します。
